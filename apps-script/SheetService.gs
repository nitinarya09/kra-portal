function submitFormData(payload) {
  ...
  for (var i = 0; i < domainSheets.length; i++) {
    ...
    if (sheet) {
      ...
      // Append new data rows with dynamic header column sync
      var lastCol = Math.max(sheet.getLastColumn(), 1);
      var headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
      var rows = payload.data[domainSheets[i]] || [];

      // Auto-sync missing header columns if payload contains keys not yet in sheet header
      if (rows.length > 0) {
        var firstRowKeys = Object.keys(rows[0]);
        for (var k = 0; k < firstRowKeys.length; k++) {
          var keyName = firstRowKeys[k];
          if (keyName !== 'rowLabel' && headers.indexOf(keyName) === -1) {
            headers.push(keyName);
            sheet.getRange(1, headers.length).setValue(keyName);
          }
        }
      }
      ...
      for (var r = 0; r < rows.length; r++) {
        var rowData = headers.map(function(h) {
          ...
          return rows[r][h] !== undefined && rows[r][h] !== null ? rows[r][h] : "";
        });
...
function isMatchingSubmission(rowSec, rowFy, rowQtr, targetSec, targetFy, targetQtr) {
  var s1 = String(rowSec || '').trim().toUpperCase();
  var s2 = String(targetSec || '').trim().toUpperCase();
  if (s1 !== s2) return false;

  var f1 = String(rowFy || '').trim();
  var f2 = String(targetFy || '').trim();
  if (f1 && f2 && f1 !== f2) return false;

  var q1 = String(rowQtr || '').trim();
  var q2 = String(targetQtr || '').trim();
  var q1Name = q1.split(' ')[0];
  var q2Name = q2.split(' ')[0];

  return (q1 === q2 || q1Name === q2Name || q1.indexOf(q2Name) === 0 || q2.indexOf(q1Name) === 0);
}
...
function approveSubmission(token, section, fy, quarter) {
  ...
  var prevStatus = "";
  for (var i = data.length - 1; i >= 1; i--) {
    if (isMatchingSubmission(data[i][0], data[i][1], data[i][2], section, fy, quarter)) {
      prevStatus = data[i][3];
